gsUI <- \(id) {
  ...
  #   )
  # )
  # Define meaningful tab names
  tab_names <- c("General Health Status",
                 "Overall progress towards UHC",
                 "Access to health services",
                 "Underlying causes of death",
                 "Morbidity patterns of disease burden"
                 )
  page_sidebar(
    title = "Example Dashboard",
    sidebar = sidebar(
      # Include scoped custom CSS styles
      tags$style(HTML("
      #mynav-tab .nav-link {
      ...
      #mynav-tab .nav-link.active {
        background-color: #27AAE1 !important;
        color: #fff !important; /* Active state with blue background and white text */
      }
      #mynav-tab .nav-link:hover {
        background-color: #27AAE1 !important;
        color: #fff !important; /* Hover state with blue background and white text */
      }
    ")),
      ...
      div(
        ...
        lapply(
          seq_along(tab_names),
          \(i) tags$a(
            class = paste0("nav-link", ifelse(i == 1, " active", "")),
            id = paste0("tab-", i, "-tab"),
            `data-toggle` = "pill",
            href = paste0("#tab-", i, "-content"),
            role = "tab",
            `aria-controls` = paste0("tab-", i, "-content"),
            `aria-selected` = i == 1,
            tab_names[i]
          )
        )
    ...
    div(
      ...
      lapply(
        seq_along(tab_names),
        \(i) div(
          class = paste0("tab-pane fade", ifelse(i == 1, " show active", "")),
          id = paste0("tab-", i, "-content"),
          role = "tabpanel",
          `aria-labelledby` = paste0("tab-", i, "-tab"),
          
          if (i == 1) {
            div(
              card(
                full_screen = TRUE,
                card_title(tab_names[i], style = "text-align:center; color:black;"),
                page_fluid(
                  layout_columns(!!!vbs,
                                 col_widths = 3
                  ))
                ...
                navset_bar(
                  ...
                  nav_panel("Bar plot",
                            sliderTextInput(
                              ...
                              "Select month:",
                              grid = TRUE,
                              choices = format(seq(as.Date("2024-07-01"), as.Date("2025-12-01"), by = "month"), "%B %Y"),
                              selected = format(as.Date("2024-07-01"), "%B %Y"),
                              from_max = format(as.Date("2024-10-01"), "%B %Y")
                            ),
                              br(),
                            highchartOutput(ns("bar_under_five")),
                            textOutput(ns("date"))
              ...
              card(
                ...
                navset_bar(
                  ...
                  nav_panel("Bar plot",
                            ...
                            br(),
                            highchartOutput(ns("bar_over_five"))
                ...
                card(
                  ...
                  navset_bar(
                    ...
                    nav_panel("Bar plot",
                              ...
                              br(),
                              highchartOutput(ns("bar_morb_under_five"))
                              )
                  )),
                card(
                  card_title(
                  ...
                  navset_bar(
                    ...
                    nav_panel("Bar plot",
                              ...
                              br(),
                              highchartOutput(ns("bar_morb_over_five"))
...
## Under five-mortality bar--------------------------------------------------------------

output$bar_under_five <- renderHighchart({
  
  df <- readRDS("RData/top_mortality_causes.RDS") |>
    dplyr::filter(as.Date(date) == lubridate::my(input$date_slider1)) |> 
    dplyr::filter(type == "Under 5 years")
  ...
  df |> 
    ...
    hc_tooltip(
      useHTML = TRUE,
      formatter = JS("function() {
        return this.point['Cause Mortality'] + '<br/><b>Number:</b> ' + this.point['Numbers'];
      }")
...
## Over five-mortality bar--------------------------------------------------------------

output$bar_over_five <- renderHighchart({
  
  df <- readRDS("RData/top_mortality_causes.RDS") |>
    dplyr::filter(as.Date(date) == lubridate::my(input$date_slider2)) |> 
    dplyr::filter(type == "Over 5 years")
...
## Under five-morbidity bar--------------------------------------------------------------

output$bar_morb_under_five <- renderHighchart({
  
  df <- readRDS("RData/top_morbidity_causes.RDS") |> 
    dplyr::filter(as.Date(date) == lubridate::my(input$morb_date_slider1)) |> 
    dplyr::filter(type == "Under 5 years")
  
  df |> 
    hchart(
      hcaes(
        x = `Cause of Morbidity`,
        y = Numbers,
      ),
      type = "bar",
      color = "#27AAE1"
    ...
    hc_tooltip(
      useHTML = TRUE,
      formatter = JS("function() {
        return this.point['Cause of Morbidity'] + '<br/><b>Number:</b> ' + this.point['Numbers'];
      }")
...
## Over five-morbidity bar--------------------------------------------------------------

output$bar_morb_over_five <- renderHighchart({
  
  df <- readRDS("RData/top_morbidity_causes.RDS") |>
    dplyr::filter(as.Date(date) == lubridate::my(input$morb_date_slider2)) |> 
    dplyr::filter(type == "Over 5 years")
